...
def compute_derived(df):
    ...
    # Operational metrics
    df["Production_Efficiency"] = df.apply(lambda r: r["Units_Sold"]/r["Units_Produced"] 
                                           if r["Units_Produced"]>0 else np.nan, axis=1)
    df["Margin_per_unit"] = df["Unit_Price"] - df["Cost_Per_Unit"] if all(x in df.columns for x in ["Unit_Price","Cost_Per_Unit"]) else np.nan
    df["Profit_Share"] = df["Profit"] / df["Profit"].sum() if "Profit" in df.columns else np.nan
    return df
...
avg_eff = filtered["Production_Efficiency"].median()
profit_margin = (total_profit / total_rev) * 100 if total_rev>0 else 0
overproduction_risk = 1 - avg_eff if not np.isnan(avg_eff) else np.nan

k1.metric("Total Revenue", f"GHS {total_rev:,.2f}", delta=f"GHS {total_profit:,.2f}")
k2.metric("Total Cost", f"GHS {total_cost:,.2f}")
k3.metric("Total Profit", f"GHS {total_profit:,.2f}")
k4.metric("Median Production Efficiency", f"{avg_eff:.2%}" if not np.isnan(avg_eff) else "N/A")
...
prod_summary = filtered.groupby("Product_Name").agg(
    ...
    Profit_Share=("Profit_Share","mean")
).round(2)
...
def generate_recommendations(df, threshold):
    ...
    if total_cost / total_rev > 0.8:
        recs.append("High cost-to-revenue ratio → renegotiate suppliers or reduce waste.")
...
with col1:
    ...
    # Profit vs Threshold with Action Classification
    profit_df = filtered.groupby("Product_Name")["Profit"].sum().reset_index().sort_values("Profit", ascending=False)
    def classify(p): return "Scale" if p>profit_thr else ("Improve" if p>0 else "Reconsider")
    profit_df["Action"] = profit_df["Profit"].apply(classify)
    fig2 = px.bar(profit_df, x="Product_Name", y="Profit", color="Action", color_discrete_map={"Scale":"green","Improve":"yellow","Reconsider":"red"})
    fig2.add_hline(y=profit_thr, line_dash="dash", line_color="red", annotation_text=f"Profit Threshold = GHS {profit_thr:,.2f}", annotation_position="top right")
...
# -------------------------
# Hidden ML Model (Predict High-Profit Products)
# -------------------------
...
df_ml["High_Profit_Probability"] = pipeline.predict_proba(X)[:,1]
high_profit_products = df_ml[df_ml["High_Profit_Probability"]>=0.5]["Product_Name"].unique()

st.subheader("Predicted High-Profit Products")
if len(high_profit_products) > 0:
    for p in high_profit_products:
        st.markdown(f"✅ **{p}** predicted as high-profit product (≥50% probability).")
else:
    st.markdown("No products predicted as high-profit at this threshold.")

st.caption("Enhanced SME Dashboard: KPIs, charts, actionable recommendations, product patterns, and predictive analytics.")
